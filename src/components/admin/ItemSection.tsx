...
const SortableItem: React.FC<{
  ...
  return (
    <motion.div
      ...
      style={style}
      layout
      className={`flex flex-col sm:flex-row gap-4 sm:gap-8 py-4 sm:py-5 transition-all bg-(bg-cream) mb-2 px-3 sm:px-4 rounded-3xl border ${isDragging ? "z-50 shadow-premium border-primary scale-[1.02]" : "border-gray-50 hover:border-primary/20 shadow-soft hover:shadow-premium"} ${!item.visible ? "opacity-40 grayscale" : ""
        }`}
    >
      {/* ===== Top Row (image + info) ===== */}
...
const ItemSection: React.FC<Props> = ({ categories, subcategories, items, setPopup }) => {
  ...
  return (
    <div className="space-y-12 rounded-4xl">
      ...
      {/* Adding Form */}
      <div className="bg-white p-10 border border-gray-100 shadow-soft relative overflow-hidden group rounded-[2.5rem]">
