...
export default function Menu({
  ...
  const featuredItems = useMemo(
    () =>
      visibleItems
        .filter((item) => item.star === true || (item as any).isFeatured === true)
  ...
  const availableCategories = useMemo(() => {
    return categories
      .filter((category) => {
        if (category.visible === false || category.available === false) return false;
        return visibleItems.some((item) => item.categoryId === category.id);
      })
      .sort((a, b) => (a.order ?? 0) - (b.order ?? 0));
  ...
  const filteredItems = useMemo(() => {
    ...
    return visibleItems
      ...
      .sort((a, b) => (a.order ?? 0) - (b.order ?? 0));
  }, [visibleItems, deferredSearchTerm]);
                ...
                >
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-gray-500 max-w-xs mx-auto">
                      {t("menu.empty_menu_desc") ||
                        "نحن نقوم بتجهيز تشكيلتنا اللذيذة. يرجى التحقق مرة أخرى قريباً."}
                    </p>
                  </div>
                </motion.div>
              ) : (
